    ...
    <div class="container mt-4">
        ...
        <ul class="nav nav-tabs mb-3">
            <li class="nav-item" v-for="tab in tabs" :key="tab">
                <a class="nav-link" :class="{ active: currentTab === tab }" href="#" @click.prevent="currentTab = tab">
                    {{ tab }}
                </a>
            </li>
        </ul>
        <div v-if="ordersData[currentTab] && ordersData[currentTab].length > 0">
            <div v-for="order in ordersData[currentTab]" :key="order.id" class="card mb-3 order-card">


                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Order #{{ order.id }}</h5>

                    <button class="btn btn-primary btn-sm mx-2" @click="pay(order)">Thanh toán</button>
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <p class="card-text"><strong>Order Code:</strong> {{ order.orderCode || 'N/A' }}</p>
                            <p class="card-text"><strong>Final Total:</strong> ${{ order.finalTotal.toFixed(2) }}</p>
                            <p class="card-text"><strong>Payment Method:</strong> {{ order.paymentMethodName }}</p>
                            <p class="card-text"><strong>Shipping Method:</strong> {{ order.shippingMethodName }}</p>
                        </div>
                        <div class="col-md-6">
                            <p class="card-text"><strong>Shipping Status:</strong>
                                <span :class="getShippingStatusClass(order.shippingStatus)">
                                    {{ order.shippingStatus }}
                                </span>
                            </p>
                            <p class="card-text"><strong>Order Info:</strong> {{ order.orderInfor }}</p>
                            <p class="card-text"><strong>Note:</strong> {{ order.note || 'No note' }}</p>
                        </div>
                    </div>
                    <hr>
