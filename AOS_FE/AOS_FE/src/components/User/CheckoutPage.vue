        ...
        <div class="col-lg-7">
          <div class="form-box p-4 bg-white rounded shadow-sm">
            ...
            <div v-if="currentTab === 0">
              <div class="mb-4">
                ...
                >
                  ...
                  <div class="d-flex align-items-center">
                    <span
                      class="badge bg-light text-danger border border-danger me-3"
                      v-if="defaultAddressData?.default"
                    >
                      Mặc Định
                    </span>
                    <button class="btn btn-link text-primary p-0" @click="goToAddress">
                      Thay Đổi
                    </button>
                  </div>
                </div>
              ...
              <div class="mb-3">
                ...
                <div v-if="isCouponApplicable" class="text-success mt-1">
                  Đã áp dụng mã: {{ selectedCoupon.code }} - Giảm
                  {{ selectedCoupon.discountValue
                  }}{{ selectedCoupon.discountType === "PERCENT" ? "%" : "₫" }}
                </div>
                <div v-else-if="selectedCoupon" class="text-warning mt-1">
                  * Đơn hàng chưa đủ
                  {{ selectedCoupon.minOrderAmount?.toLocaleString?.() || 0 }}₫ để áp dụng
                  mã
                </div>
              </div>

              <button class="btn btn-warning w-100 fw-bold" @click="currentTab = 1">
                Bước tiếp theo
              </button>
            </div>
            ...
            <div v-if="currentTab === 1">
              ...
              <button class="btn btn-primary w-100 fw-bold" @click="currentTab = 2">
                Tiếp tục
              </button>
            </div>
            ...
            <div v-if="currentTab === 2">
              ...
              <ul class="list-unstyled">
                ...
                <li>
                  <strong>Số điện thoại:</strong> {{ defaultAddressData?.phone || "—" }}
                </li>
                <li><strong>Địa chỉ:</strong> {{ fullAddress }}</li>
                <li>
                  <strong>Giảm giá:</strong> {{ selectedCoupon?.code || "Không áp dụng" }}
                </li>
              ...
              <button class="btn btn-success mt-3 w-100 fw-bold" @click="confirmOrder">
                Xác nhận đặt hàng
              </button>
            </div>
          </div>
...
async function confirmOrder() {
  ...
  try {
    ...
    selectedProducts.value.forEach((item) => {
      authService.updateCart(-item.quantity); // Reset số lượng sau khi đặt hàng
    });
...
onMounted(async () => {
  ...
  console.log(
    "✅ Đã tải danh sách phương thức thanh toán:",
    dropdownPaymentMethods.value
  );
