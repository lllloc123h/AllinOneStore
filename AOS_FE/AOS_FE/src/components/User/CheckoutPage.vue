                                    ...
                                    <div v-for="method in dropdownPaymentMethods" :key="method.id" :class="[
                                        ...
                                        <div class="payment-method-content">
                                            ...
                                            <div class="payment-radio">
                                                <input type="radio" :value="method" v-model="paymentMethod"
                                ...
                                <div class="confirmation-sections">
                                    ...
                                    <div class="confirmation-card">
                                        ...
                                        <div v-for="(group, groupId) in groupedProducts.comboGroups" :key="groupId"
                                            ...
                                            <div class="fw-bold text-primary mb-2">🎁 Combo: {{
                                                group.items[0].promotions.name }}
                                            </div>
                                            <ul class="list-unstyled ps-3 small">
                                                <li v-for="item in group.items" :key="item.id"
                                                    class="d-flex align-items-start mb-2">
                                                    <img :src="item.image" alt="ảnh sản phẩm" class="me-2 rounded"
                                            ...
                                            <div class="ps-3">
                                                📦 Số lượng combo: <strong>{{ group.comboQty }}</strong><br />
                                                💰 Giá gốc: <s>{{ group.originalTotal.toLocaleString() }}₫</s><br />
                                                🏷️ Tiết kiệm:
                                                <span class="fw-semibold text-success">
                                                    {{ (group.originalTotal - group.comboPrice *
                                        ...
                                        <div v-if="groupedProducts.discountItems.length" class="mt-3">
                                            <div v-for="item in groupedProducts.discountItems" :key="item.id"
                                                ...
                                                <div class="fw-bold text-warning mb-2">🔥 Khuyến mãi: {{
                                        ...
                                        <div v-if="groupedProducts.normalItems.length" class="mt-3">
                                            <div v-for="item in groupedProducts.normalItems" :key="item.id"
                                                class="mb-3 border p-3 rounded">
                                                <div class="fw-bold text-secondary mb-2">🛒 Sản phẩm:</div>
                                                <ul class="list-unstyled ps-3 small">
                                                    <li class="d-flex align-items-start mb-2">
                                                        <img :src="item.image" alt="ảnh sản phẩm" class="me-2 rounded"
                                                            style="width: 48px; height: 48px; object-fit: cover;">
...
const totalPrice = computed(() => {
    ...
    const discountTotal = groupedProducts.value.discountItems.reduce(
        (sum, item) => sum + (item.price - (item.promotions?.discountValue || 0)) * item.quantity,
        0
    );
