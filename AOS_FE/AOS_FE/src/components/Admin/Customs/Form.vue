    ...
    <div class="article col-9">
      <form @submit.prevent="props.action === 'create' ? submitForm() : submitUpdateForm()">
        ...
        <div class="mb-3">
          ...
          <input :id="accountId" v-model="formData.accountId" type="number" class="form-control"
            :placeholder="`Enter accountId`" />
        </div>

        <div class="mb-3">
          <label :for="canvasJson" class="form-label text-capitalize">canvasJson</label>
          <input :id="canvasJson" v-model="formData.canvasJson" type="text" class="form-control"
            :placeholder="`Enter canvasJson`" />
        </div>

        <div class="mb-3">
          <label :for="imageUrl" class="form-label text-capitalize">imageUrl</label>
          <input :id="imageUrl" v-model="formData.imageUrl" type="text" class="form-control"
            :placeholder="`Enter imageUrl`" />
        </div>

        <div class="mb-3">
          <label :for="designName" class="form-label text-capitalize">designName</label>
          <input :id="designName" v-model="formData.designName" type="text" class="form-control"
