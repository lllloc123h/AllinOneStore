    ...
    <div class="article col-9">
      ...
      <div class="dropdown-grid">
        <div class="main-section rounded-4">
          <button class="first-button" @click="openModal(selectedProduct)">
            Thêm Biến thể
          </button>
        </div>
      </div>
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="dropdown">
                ...
                >
                  <span v-if="selectedVarriantColor">
                    {{ selectedVarriantColor.name }} ({{
                      selectedVarriantColor.description
                    }})
                  </span>
                ...
                >
                  <li
                    v-for="color in dropDownListVariants['Màu sắc']"
                    :key="color.id"
              ...
              <div class="dropdown">
                ...
                >
                  <span v-if="selectedVarriantSize">
                    {{ selectedVarriantSize.name }} ({{
                      selectedVarriantSize.description
                    }})
                  </span>
                ...
                >
                  <li
                    v-for="size in dropDownListVariants['Kích thước']"
                    :key="size.id"
              ...
              <div class="mb-3">
                ...
                <input
                  id="safetyStock"
                  v-model="selectedProduct.safetyStock"
                  type="number"
                  class="form-control"
              ...
              <div class="mb-3">
                ...
                <input
                  id="sellStart"
                  v-model="selectedProduct.sellStart"
                  type="datetime-local"
                  class="form-control"
              ...
              <div class="mb-3">
                ...
                <input
                  id="sellEnd"
                  v-model="selectedProduct.sellEnd"
                  type="datetime-local"
                  class="form-control"
            ...
            <div class="d-flex align-items-center">
              ...
              <div>
                ...
                <small class="text-muted"
                  >Ảnh: {{ item.fileNameImgOfVariant || "Không có" }}</small
                >
              </div>
            </div>
            <button class="btn btn-sm btn-outline-danger" @click="removeVariant(item.id)">
              Xóa
            </button>
          </li>
        </ul>
...
async function addToCart() {
  const isContain = list.value.filter(
    (item) => splitSku(item.sku) === selectedProduct.value.sku
  );
  ...
  try {
    const responseSaveProductItem = await api.post(
      "/admin/ProductItems",
      selectedProduct.value
    );
    const responseSaveImg = await api.post("/admin/ProductImages", {
      id: "",
      productItems: responseSaveProductItem.data.id,
      imageUrl: resultImgUpload[0].url,
    });
  } catch (error) {
    console.error("Insert failed:", error);
    notification.error({
...
watch([() => selectedVarriantColor.value, () => selectedVarriantSize.value], () => {
  mapSku.value =
    selectedVarriantColor.value?.signalSku + "-" + selectedVarriantSize.value?.signalSku;
});
watch(
  () => mapSku.value,
  () => {
