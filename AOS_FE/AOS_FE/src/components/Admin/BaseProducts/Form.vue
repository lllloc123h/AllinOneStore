    ...
    <div class="article col-9">
      <form @submit.prevent="props.action === 'create' ? submitForm() : submitUpdateForm()">
        ...
        <div class="mb-3" v-if="props.action === 'view' || props.action === 'update'">
          ...
          <input type="text" class="form-control" :value="formData.categories?.name || 'Unknown'" readonly />
        </div>
        <div class="mb-3" v-show="props.action != 'view'">
          ...
          <select id="categories" v-model="formData.categories" class="form-select">
            ...
            <option v-for="item in categoriesDropDownList" :key="item.id" :value="item.id">
              {{ item.name }}
            </option>
          </select>
        </div>
