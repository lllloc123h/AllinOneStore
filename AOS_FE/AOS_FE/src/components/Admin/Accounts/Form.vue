<template>
	<div class="container-fluid row">
		...
		<div class="article col-9">
			<div>
				<h1>
					<a :href="`/Admin/${props.TableName}`">{{ props.TableName }}</a> &gt; {{ props.action }}
				</h1>
			</div>

			<form @submit.prevent="props.action === 'create' ? submitForm : submitUpdateForm"
				class="w-100 p-3 border rounded bg-light">
				<div class="mb-3"
					:style="(props.action === 'view' || props.action === 'create') ? ' display:none;' : ''">
					<label :for="id" class="form-label text-capitalize"></label>
					<input :id="id" v-model="formData.id" v-if="props.action !== 'create'"
						:hidden="props.action === 'view'" type="number" class="form-control"
						:placeholder="`Enter id`" />
				</div>

				<div class="mb-3">
					<label :for="email" class="form-label text-capitalize"></label>
				...
				<div class="mb-3">
					...
					<input :id="password" v-model="formData.password" type="text" class="form-control"
						:placeholder="`Enter password`" />
				</div>

				<div class="mb-3">
					<label :for="fullname" class="form-label text-capitalize"></label>
					<input :id="fullname" v-model="formData.fullname" type="text" class="form-control"
						:placeholder="`Enter fullname`" />
				</div>

				<div class="mb-3">
					<label :for="avatar" class="form-label text-capitalize"></label>
					<input :id="avatar" v-model="formData.avatar" type="text" class="form-control"
						:placeholder="`Enter avatar`" />
				</div>

				<div class="mb-3">
					<label :for="phone" class="form-label text-capitalize"></label>
					<input :id="phone" v-model="formData.phone" type="text" class="form-control"
						:placeholder="`Enter phone`" />
				</div>

				<div class="mb-3">
					<label :for="averageOrderValue" class="form-label text-capitalize"></label>
					<input :id="averageOrderValue" v-model="formData.averageOrderValue" type="number"
						class="form-control" :placeholder="`Enter averageOrderValue`" />
				</div>

				<div class="mb-3">
					<label :for="userRank" class="form-label text-capitalize"></label>
					<input :id="userRank" v-model="formData.userRank" type="text" class="form-control"
						:placeholder="`Enter userRank`" />
				</div>

				<div class="mb-3">
					<label :for="totalSpent" class="form-label text-capitalize"></label>
					<input :id="totalSpent" v-model="formData.totalSpent" type="number" class="form-control"
						:placeholder="`Enter totalSpent`" />
				</div>

				<div class="mb-3">
					<label :for="totalOrder" class="form-label text-capitalize"></label>
					<input :id="totalOrder" v-model="formData.totalOrder" type="number" class="form-control"
						:placeholder="`Enter totalOrder`" />
				</div>

				<div class="mb-3">
					<label :for="loyaltyPoint" class="form-label text-capitalize"></label>
					<input :id="loyaltyPoint" v-model="formData.loyaltyPoint" type="number" class="form-control"
						:placeholder="`Enter loyaltyPoint`" />
				</div>

				<div class="mb-3">
					<label :for="createdAt" class="form-label text-capitalize"></label>
					<input :id="createdAt" v-model="formData.createdAt" type="date" class="form-control"
						:placeholder="`Enter createdAt`" />
				</div>

				<div class="mb-3">
					<label :for="updatedAt" class="form-label text-capitalize"></label>
					<input :id="updatedAt" v-model="formData.updatedAt" type="date" class="form-control"
						:placeholder="`Enter updatedAt`" />
				</div>

				<button type="submit" :disable="props.action === 'view'" class="btn btn-primary">
					{{ props.action === 'create' ? 'Create' : (props.action === 'update' ? 'Update' : 'view')
					}}</button>
			</form>
		</div>
	</div>
