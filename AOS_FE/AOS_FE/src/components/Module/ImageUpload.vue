  ...
  <div class="row mt-4 g-4">
    ...
    <div class="col-3" v-for="(item, index) in fileView" :key="index">
      <div style="border: 0px" class="card position-relative overflow-hidden rounded-4">
        ...
        <img v-if="item.type === 'image'" class="card-img rounded-4 custom-shadow"
          style="height: 450px; object-fit: cover" :src="item.url" alt="Card image" />
        <video v-else class="card-img rounded-4 custom-shadow" style="height: 220px; object-fit: cover" :src="item.url"
          controls></video>
        <div v-if="!item.valid" style="
            color: red;
            position: absolute;
            bottom: 8px;
            left: 8px;
    ...
    <button @click="uploadAllFiles" :disabled="!canUpload || isUploading" style="min-width: 120px; position: relative">
      <span v-if="!isUploading">Upload</span>
      <span v-else>
        <svg style="width: 20px; height: 20px; vertical-align: middle; margin-right: 6px" viewBox="0 0 50 50">
          <circle cx="25" cy="25" r="20" fill="none" stroke="#10c6c3" stroke-width="5" stroke-linecap="round"
...
const errorMsg = ref("");
const emit = defineEmits(["update-avatar", "update-main-image-url", "result-uploaded"]);
